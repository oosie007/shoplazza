...
export async function POST(request: NextRequest) {
  ...
  for (const line of lineItems) {
    ...
    const productId = product.product_id != null ? String(product.product_id) : (product.productId != null ? String(product.productId) : "");
    const price = parseFloat(product.price ?? product.price_amount ?? "0") || 0;
