...
function readInput() {
  ...
  var reduced = inputChunks.reduce(function (context, chunk) {
...
function run(input) {
  ...
  lineItems.forEach(function (lineItem) {
    ...
    if (title === "Item protection") {
      ...
      var price = parseFloat(product.price || product.price_amount || lineItem.price || lineItem.final_price || "0") || 0;
  ...
  if (protectionLineId) {
    ...
    runResult.operation.update.push({ id: protectionLineId, price: { adjustment_fixed_price: adjustment } });
  }
  return runResult;
}
`;
