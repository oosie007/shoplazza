...
- **403 Forbidden on list or bind:** The store’s access token doesn’t have permission to use the Function/Cart Transform API. Add the required scope(s) in `src/lib/shoplazza/auth.ts` (e.g. `read_function`, `write_function` – check [Shoplazza Access Scopes](https://www.shoplazza.dev/reference/access-scopes-copy) for the exact names). Then **reinstall the app** (or have the merchant re-authorize) so the store gets a new token with the new scopes. Until then, bind will keep returning 403.
