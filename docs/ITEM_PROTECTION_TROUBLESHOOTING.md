...
- **403 Forbidden on list or bind:** The store’s access token doesn’t have permission to use the Function/Cart Transform API. Add the required scope(s) in `src/lib/shoplazza/auth.ts`; then **reinstall the app** so the store gets a new token. Until then, bind will keep returning 403.
